#all write Codes By Team Afrotoo @NUNJJ
#By Hussein @NUNUU
import asyncio
import random
import json
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @soursafrotoo 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="᯽︙ THEME BY : @soursafrotoo 🎊",parse_mode="html")
  await theme.delete()
...
async def Hussein(event):
    ...
    sed = event.pattern_match.group(2)
    sedthon_links[sed] = url
    await event.edit(f"**᯽︙ تم اضافة البصمة {sed} بنجاح ✓ **")
    with open('aljoker_links.json', 'w') as file:
        json.dump(sedthon_links, file)

@l313l.on(admin_cmd(outgoing=True, pattern="?(.*)"))
async def Hussein(event):
    sed = event.pattern_match.group(1)
    Afrotoo = await reply_id(event)
    if sed in sedthon_links:
        url = sedthon_links[sed]
        await event.client.send_file(event.chat_id, url, parse_mode="html", reply_to=Afrotoo)
        await event.delete()

@l313l.ar_cmd(pattern="ازالة(?:\s|$)([\s\S]*)")
async def delete_sedthon(event):
    sed = event.pattern_match.group(1)
    if sed in sedthon_links:
        del sedthon_links[sed]
        with open('sedthon_links.json', 'w') as file:
            json.dump(sedthon_links, file)
        await event.edit(f"**᯽︙ تم حذف البصمة '{sed}' بنجاح ✓**")
    else:
        await event.edit(f"**᯽︙ هذه البصمة '{sed}' غير موجودة في القائمة**")
        
@l313l.on(admin_cmd(outgoing=True, pattern="قائمة الميمز"))
async def list_sedthon(event):
    if sedthon_links:
        message = "**᯽︙ قائمة تخزين اوامر الميمز:**\n"
        for sed, url in sedthon_links.items():
            message += f"- البصمة : .`{sed}`\n"
    else:
        message = "**᯽︙ لاتوجد بصمات ميمز مخزونة حتى الآن**"
    await event.edit(message)
    
@l313l.on(admin_cmd(outgoing=True, pattern="ازالة_البصمات"))
async def delete_all_sedthon(event):
    global sedthon_links
    sedthon_links = {}
    with open('sedthon_links.json', 'w') as file:
        json.dump(sedthon_links, file)
