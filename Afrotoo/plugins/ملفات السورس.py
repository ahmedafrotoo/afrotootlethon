...
@l313l.ar_cmd(
    ...
    info={
        "header": "To list all plugins in Afrotoo.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in Afrotoo"
    cmd = "ls Afrotoo/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[عفرتو](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@l313l.ar_cmd(
    ...
    info={
        "header": "To list all environment values in Afrotoo.",
        "description": "to show all heroku vars/Config values in your Afrotoo",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in Afrotoo"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
