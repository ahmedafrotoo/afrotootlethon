...
from Afrotoo import HEROKU_APP, UPSTREAM_REPO_URL, l313l
...
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/Afrotoo"
...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**᯽︙ قام مطورين السورس بتحديث عفرتو**\n᯽︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس عفرتو بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/Afrotoo", force=True)
    ...
    if build_status.status == "failed":
        ...
        return
    try:
        remote.push("Afrotoo:main", force=True)
    except Exception as error:
        await event.edit(f"{txt}\n**هذا هو سجل الاخطاء:**\n`{error}`")
        return repo.__del__()
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس عفرتو انـتـظـر**")
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("Afrotoo", origin.refs.Afrotoo)
        repo.heads.Afrotoo.set_tracking_branch(origin.refs.Afrotoo)
        repo.heads.Afrotoo.checkout(True)
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس عفرتو ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس عفرتو انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ جارِ تحديث ريبو التنصيب لسورس عفرتو **")
    off_repo = "https://github.com/ahmedafrotoo/afrotootlethon"
    os.chdir("/app")
    try:
        txt = (
        ...
        origin = repo.create_remote("upstream", off_repo)
        origin.fetch()
        repo.create_head("Afrotoo", origin.refs.master)
        repo.heads.Afrotoo.set_tracking_branch(origin.refs.master)
        repo.heads.Afrotoo.checkout(True)
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("**᯽︙ جارِ اعادة تنصيب سورس عفرتو, انتظر قليلاً ..**")
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        conf = "الان"
        event = await event.reply("**᯽︙ يتم البحث عن تحديث عفرتو تحديث بامر المطور اجبارياً**")
        off_repo = UPSTREAM_REPO_URL
        force_update = False
        ...
        if conf == "الان":
            await event.edit("** ᯽︙ يتم تحديث سورس عفرتو بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
