...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗦𝞝 ΆҒŔΌŢΌΌ- اوامــر المطـور المســاعد](t.me/soursafrotoo) 𓆪\n\n"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝗦𝞝𝗗 :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        zedevent = await edit_or_reply(event, "**✾╎تـم تفعيـل التحكـم للمطـوريـن لـ الاوامـر الآمـنـه .. بنجـاح🧑🏻‍💻✅**")
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your sthon__.\n"
                )
            else:
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت عفرتو انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "كامل" or input_str[0] == "الكل":
        ...
        zedevent = await edit_or_reply(
            event, "**✾╎تـم تعطيـل التحكـم للمطـوريـن لـ الاوامـر الآمـنـه .. بنجـاح🧑🏻‍💻✅**"
        )
        flagcmds = (
            PLG_INFO["botcontrols"]
            + PLG_INFO["الوقتي"]
...
# Copyright (C) 2023 SED-THON . All Rights Reserved
