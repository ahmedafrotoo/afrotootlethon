    ...
    if len(l313l) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await Afrotoo.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and Afrotoo.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            Afrotoo = await event.client.send_file(
                event.chat_id, Afrotoo, caption=Afrotoo.text
            )
            await _catutils.unsavegif(event, Afrotoo)
        ...
        if BOTLOG:
            ...
            Afrotoo = await event.client.send_file(BOTLOG_CHATID, JoKeRUB)
            await _catutils.unsavegif(event, Afrotoo)
        return
    elif event.reply_to_msg_id and JoKeRUB.text:
        spam_message = Afrotoo.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    Afrotoo = await event.get_reply_message()
    ...
    await spam_function(event, Afrotoo, l313l, sleeptimem, sleeptimet)
