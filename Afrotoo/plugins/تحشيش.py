...
async def permalink(mention):
    ...
    if not user:
        return
    main = user.last_name.replace("\u2060", "") if user.last_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"🚻 ** ᯽︙  المستخدم => • ** [{main}](tg://user?id={user.id}) \n ☑️ **᯽︙  تم رفعها مرتك بواسطه  :**{my_mention} 👰🏼‍♀️.\n**᯽︙  يلا حبيبي امشي نخلف بيبي 👶🏻🤤** ")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 6190055518:
        return await edit_or_reply(mention, f"**- لكك دي هذا المطور**")
    main = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
@l313l.on(admin_cmd(pattern="بلال(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"سماحة السيد بلال علي مطور سورس سيدثون @NUNUU")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ الحلو** 「[{main}](tg://user?id={user.id})」 \n**᯽︙  تـم رفعه مميز بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"᯽︙ ** لقد تم زواجك/ج من : **[{main}](tg://user?id={user.id}) 💍\n**᯽︙  الف الف مبروك الان يمكنك اخذ راحتك ** ")
...
lMl10l = [6190055518, 6190055518, 6190055518, 6190055518]
    ...
    if event.reply_to and event.sender_id in lMl10l:
       ...
       if owner_id == l313l.uid:
           if event.message.message == "منصب؟":
               ...
               await event.reply("** السيد بلال @lUl_UI تاج راسي ❤️**")
...
async def permalink(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙الهمسة من المستخدم [{main}](tg://user?id={user.id}) تم كشفها بنجاح ✓**\n**᯽︙  الهمسة هي : {lMl10l} ** ")
...
async def _(event):
    ...
    if user.id == 6190055518:
        await event.reply("اهلا بك عفرتو مطوري\nقناة السورس:  @hsoursafrotoo")
