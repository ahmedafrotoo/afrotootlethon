...
from Afrotoo import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Afrotoo/plugins/{shortname}.py")
        checkplugins(path)
        name = "Afrotoo.plugins.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("᯽︙تم بنجاح تحميل ملف عفرتو " + shortname)
    else:
        if plugin_path is None:
            path = Path(f"Afrotoo/plugins/{shortname}.py")
            name = f"Afrotoo.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Afrotoo.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف عفرتو✓" + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Afrotoo/plugins/assistant/{shortname}.py")
        name = "Afrotoo.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["Afrotoo.plugins.assistant" + shortname] = mod
