...
class NewMessage(events.NewMessage):
    ...
    def filter(self, event):
        ...
        if gvarstatus("blockedfrom") == "yes":
            event._client.loop.create_task(edit_or_reply(event, "**᯽︙ لا يمكنك أستخدام سورس عفرتو لأنك محظور من قبل المطور**"))
            return
        return event
...
async def safe_check_text(msg):  # sourcery no-metrics
    if not msg:
        return False
    msg = str(msg)
    from .session import l131l

    phone = str((await l131l.get_entity(l131l.uid)).phone)
    return bool(
        (
