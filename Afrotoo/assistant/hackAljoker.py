...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 23256234, "d8cb7e0d5e27051be5e6ba5c8664f1c1") as X:
    bot = client = X
    try: 
...
'''
mm = '''
قم بلأنضمام الى قناة الجوكر @E9N99
'''
...
keyboard = [
  ...
  [
    Button.url("سورس سيدثون ", "https://t.me/E9N99")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/NUNUU")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
          ...
          await event.reply(f"حدث خطأ قم بتوجيه الرسالة للمطور @NUNUU\n{i}")
